 ...
 *
 * ---------------------------------------------------------------------------
 * NO FFMPEG?  Pass --wav and it writes .wav instead. They are about 8x bigger
 * but need no extra tool:   node tools/make-audio.mjs --wav
 * ---------------------------------------------------------------------------
 */
...
const WAV_ONLY = process.argv.includes('--wav');
const EXT = WAV_ONLY ? 'wav' : 'mp3';
...
if (!WAV_ONLY) {
  ...
  catch { console.error('ffmpeg not found. Install it, or run with --wav.'); process.exit(1); }
}
