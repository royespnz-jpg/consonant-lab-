...
interface Student { name: string; section: string; }
...
function viewIdentity(): void {
  ...
  const go = () => {
    ...
    const section = $<HTMLInputElement>('#ssec').value.trim();
    if (!name || !section) { $('#iderr').hidden = false; return; }
    state.student = { name, section };
    saveState();
    viewHome();
  };
  $('#idgo').addEventListener('click', go);
  ['#sname', '#ssec'].forEach((sel) =>
...
function wireSettings(): void {
  ...
  const paint = () => {
    ...
    $('#engine-bank-wrap').hidden = !bank.available;
    if (bank.available) {
