...
const ClipStore = {
  ...
  open(): Promise<IDBDatabase | null> {
    ...
    return new Promise((resolve) => {
      ...
      try { req = indexedDB.open('consonant-lab-audio', 1); }
...
class KokoroEngine {
  ...
  load(): Promise<void> {
    ...
    this.loading = (async () => {
      ...
      // WebGPU generates in a fraction of the time WASM needs. Where it is
      // missing (or refuses at run time) we fall back to WASM below.
      ...
      // Some drivers advertise WebGPU and then fail to compile the model.
      if (this.backend === 'webgpu') {
        try {
          const mod: any = await import(/* @vite-ignore */ KOKORO_CDN);
          this.backend = 'wasm';
          this.tts = await mod.KokoroTTS.from_pretrained(KOKORO_MODEL, { dtype: 'q8', device: 'wasm' });
          this.status = 'ready';
          this.progress = 100;
          this.onchange?.();
          return;
...
function wireSettings(): void {
  ...
  const paint = () => {
    ...
    if (!bank.available) {
      $('#bank-note').textContent = bank.loaded
        ? 'No clips uploaded yet. Run tools/make-audio.mjs and commit the audio folder to switch this on.'
    ...
    if (!useK) { box.hidden = true; }
      ...
      if (k.status === 'loading') {
        ...
        box.innerHTML = k.backend === 'webgpu'
          ? 'Kokoro is loaded and running on the graphics card &mdash; the fast path.'
          : `Kokoro is loaded, but this browser has no WebGPU, so it runs on the CPU and
             takes a few seconds per word. <small>Recorded clips are the better option here.</small>`;
