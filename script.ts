...
const blankState = (): SaveState => ({
  ...
  settings: {
    ...
    studioUrl: DEFAULT_STUDIO_URL, studioVoice: '', studioModel: 'omnivoice',
  },
});
...
class StudioEngine {
  ...
  private base(): string {
    return (state.settings.studioUrl || DEFAULT_STUDIO_URL).replace(/\/+$/, '');
  }
  ...
  probe(force = false): Promise<void> {
    ...
    this.probing = (async () => {
      ...
      try {
        const res = await fetch(`${this.base()}/v1/audio/voices`, { signal: ctrl.signal });
        if (!res.ok) throw new Error(`the server answered ${res.status}`);
        const body = await res.json();
        this.voices = (body.voices || []).map((v: any) => ({
          id: String(v.voice_id),
          name: String(v.name || v.voice_id),
          kind: v.type === 'openai_alias' ? 'built-in' : (v.language || 'cloned'),
        }));
        if (!this.voices.length) throw new Error('it has no voices yet — create one in the app');
...
function wireSettings(): void {
  ...
  const paint = () => {
    ...
    studioUrl.value = state.settings.studioUrl || DEFAULT_STUDIO_URL;
    ...
    if (useS) {
      ...
      if (st.status === 'error') { out.hidden = false; out.className = 'help connout bad'; out.textContent = st.error; }
        ...
        out.textContent = `Connected. ${st.voices.length} voice${st.voices.length === 1 ? '' : 's'} available.`;
