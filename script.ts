...
class Speaker {
  ...
  async sequence(items: string[], gapMs = 520, rateMul = 1): Promise<void> {
...
function renderQuestion(): void {
  ...
  const play = (rateMul?: number) => {
    ...
    void speaker.sequence(q.audio, 520, rateMul ?? q.rate ?? 1).then(() => {
  ...
  setTimeout(() => play(), 260);
  // Warm the next question's audio while the student is thinking.
