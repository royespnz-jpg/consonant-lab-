...
class StudioEngine {
   ...
   *
   * Same origin comes first and is the reason this needs no setting up: open
   * the page through the tunnel that publishes VoiceStudio (ngrok http 3900)
   * and the page and the API share an origin, so there is no address to type
   * and no CORS list to edit. Loopback is second, for a teacher working on the
   * machine VoiceStudio is running on.
   */
  private candidates(): string[] {
    const list: string[] = [];
    if (location.origin && /^https?:/.test(location.origin)) list.push(location.origin);
    if (!list.includes(DEFAULT_STUDIO_URL)) list.push(DEFAULT_STUDIO_URL);
    return list;
  }

  /** One address, one answer. Throws with something worth reading. */
  private async ask(url: string): Promise<StudioVoice[]> {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), 8000);
    try {
      const res = await fetch(`${url}/v1/audio/voices`, { signal: ctrl.signal });
  ...
  private explain(e: unknown): string {
    ...
    if (/^the server answered/.test(msg) || /no voices yet/.test(msg)) return msg;
  ...
  private key(text: string, speed: number): string {
    return `vs|${this.base()}|${state.settings.studioModel}|${state.settings.studioVoice}|${speed.toFixed(2)}|${text}`;
  }
  ...
  const paint = () => {
    ...
    if (!bank.available) {
      $('#bank-note').innerHTML = bank.loaded
        ? `Not available yet &mdash; the audio files have not been made.
           <br>Open VoiceStudio, then in the project folder:
           <code>npm install</code>, <code>npm run voices</code> to see the
           voices, then <code>npm run audio -- --voice "&lt;name&gt;"</code>,
           and commit the <code>audio</code> folder it creates.`
    ...
    if (useS) {
      ...
      if (st.status === 'error') { out.hidden = false; out.className = 'help connout bad'; out.textContent = st.error; }
        ...
        out.textContent = `Connected to ${st.base()} — ${st.voices.length} `
          + `voice${st.voices.length === 1 ? '' : 's'} available.`;
