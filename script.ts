...
/* Where VoiceStudio listens by default. Loopback is the one address an https
   page may call over plain http — browsers treat it as trustworthy — so it is
   also the only one that needs no tunnel and no certificate. */
const DEFAULT_STUDIO_URL = 'http://127.0.0.1:3900';
/* Piper's http_server listens here. It sends no CORS headers whatsoever, so it
   is only reachable when the page itself is served from the same origin —
   drop the app into Piper's img/ folder and open /img/lab/index.html. */
...
type Voicing = 'voiceless' | 'voiced' | 'none';
...
interface SoundProfile {
  ...
  place: string;
  howTo: string;
  spellings: Array<{ pattern: string; note?: string; examples: string }>;
}

interface WordPair { a: string; b: string; }

interface SentencePair { a: string; b: string; }

interface SortItem {
  word: string;
  /** index into unit.sounds */
  sound: number;
  /** restrict the offered options to these unit.sounds indices */
  opts?: number[];
}

interface ReviewCard {
  kind: 'sound' | 'contrast' | 'rule';
  title: string;
  body?: string;
  sound?: SoundProfile;
  rows?: Array<{ head: string; items: string }>;
  callout?: string;
}
...
interface Unit {
  ...
  review: ReviewCard[];
  pairs: WordPair[];
  /** 3-way discrimination sets — the backbone of the hard minimal-pair drills */
  triples: string[][];
  sentences: SentencePair[];
  sort: SortItem[];
}

type ActivityKind = 'pairs' | 'pairsx' | 'samediff' | 'odd' | 'sentence' | 'sort';
...
interface Question {
  ...
  answer: number;
  note?: string;
  /** how many times the student may replay (the first play is free) */
  maxReplays: number;
  /** speaking-rate multiplier for this question */
  rate?: number;
  /** hide the Slower button */
  noSlow?: boolean;
}

interface ExamItem { frame: string; options: string[]; }

interface UnitProgress { reviewed: boolean; best: Record<string, number>; }

/** 'bank' = pre-generated mp3 files, the fastest and most consistent option. */
type EngineName = 'bank' | 'device' | 'kokoro' | 'studio';
...
interface SaveState {
  ...
  exam: { best: number; taken: number };
  settings: {
    engine: EngineName;
    /* Set once, the first time a clip bank is actually present, so the upgrade
       onto it happens for returning students but only ever happens once. */
    bankSeen: boolean;
    /* Where VoiceStudio is listening, and what it should speak with. */
    studioUrl: string;
    studioVoice: string;
    studioModel: string;
    bankSet: string | null;
...
const S: Record<string, SoundProfile> = {
  s: {
...
const target = (s: string): string => (s.match(/\{([^}]+)\}/) || ['', s])[1];
const markup = (s: string): string =>
  escapeHtml(s).replace(/\{([^}]+)\}/g, '<b class="tgt">$1</b>');
...
/**
 * Carrier phrases for the Expert drill, so the target word is coarticulated.
 * The phrase is chosen from the word itself rather than at random: each word
 * always gets the same frame, which keeps the variety across the set while
 * letting the clip generator produce exactly one recording per word instead
 * of one per word per phrase.
 */
const CARRIERS = [
  'Say ___ again.',
...
function carrierFor(word: string): string {
  let h = 0;
...
const blankState = (): SaveState => ({
  version: 5,
  student: null,
  units: {},
  exam: { best: 0, taken: 0 },
  settings: {
    engine: 'bank', bankSeen: false, bankSet: null, voiceURI: null,
    kokoroVoice: 'af_heart', rate: 0.9, theme: 'auto',
    studioUrl: '', studioVoice: '', studioModel: 'omnivoice',
  },
});

let state: SaveState = blankState();
...
function loadState(): void {
  ...
  for (const u of UNITS) if (!state.units[u.id]) state.units[u.id] = { reviewed: false, best: {} };

  // Anyone whose saved settings still say 'kokoro' picked it under a build that
  // produced corrupt audio. Put them back on the safe default; choosing it
  // again is one click.
  if (state.settings.engine === 'kokoro') {
    state.settings.engine = 'bank';
    saveState();
  }
}
...
const ACTIVITIES: Array<{ id: ActivityKind; name: string; blurb: string; hard?: boolean }> = [
...
const unitMastered = (u: Unit): boolean =>
  ACTIVITIES.every((a) => (state.units[u.id].best[a.id] || 0) >= PASS);
const cleared = (u: Unit, a: ActivityKind): boolean => (state.units[u.id].best[a] || 0) >= PASS;

/* ============================================================================
   6b. RESULTS REPORTING  ->  Google Apps Script -> Google Sheet
   ==========================================================================
   ...
   (mode: no-cors), so success is assumed unless the request itself throws;
   anything that fails is kept in an outbox and retried later.
   ========================================================================== */

interface Report {
  kind: 'score';
  student: string;
  /** Every question the student got wrong, so the teacher sees what to reteach. */
  misses: Miss[];
  unit: string;
  unitTitle: string;
  activity: string;
...
const reportingOn = (): boolean => REPORT_URL.trim().length > 0;

type PostResult = 'ok' | 'unconfirmed' | 'failed';

/**
 * Posts one JSON payload to the Apps Script endpoint.
 *
 * Content-Type text/plain makes this a "simple" request, so there is no CORS
 * preflight for Apps Script to fail. That means we can use normal cors mode and
 * actually READ the reply — which is the only way to know a row was written.
 * If the browser still refuses (an old deployment without the right headers),
 * we retry with no-cors so the data at least arrives, and say plainly that it
 * could not be confirmed.
 */
let eventSeq = 0;

/** Stable per-event id so a retry cannot be written twice. */
...
async function postJson(payload: object): Promise<PostResult> {
  // The cors attempt below still REACHES the server even when the browser
  // refuses to let us read the reply, so the no-cors retry would write the row
  // a second time. The id lets the sheet ignore the repeat.
  const withId = { id: eventId(), ...payload };
  const body = JSON.stringify(withId);
  const headers = { 'Content-Type': 'text/plain;charset=utf-8' };

  try {
    const res = await fetch(REPORT_URL, { method: 'POST', headers, body, redirect: 'follow' });
    if (!res.ok) return 'failed';
    const text = await res.text();
    // A sign-in page instead of our JSON means the deployment is not public.
    if (/<html/i.test(text) || /accounts\.google\.com/i.test(text)) return 'failed';
    try {
      const j = JSON.parse(text);
      return j && j.ok === false ? 'failed' : 'ok';
    } catch {
      return 'unconfirmed';
    }
  } catch {
    // Could not read the reply. Send it blind so nothing is lost.
    try {
      await fetch(REPORT_URL, { method: 'POST', mode: 'no-cors', headers, body });
      return 'unconfirmed';
    } catch {
      return 'failed';
    }
...
function readOutbox(): Report[] {
  try { return JSON.parse(localStorage.getItem(OUTBOX_KEY) || '[]') as Report[]; }
  catch { return []; }
}

function writeOutbox(rows: Report[]): void {
  try { localStorage.setItem(OUTBOX_KEY, JSON.stringify(rows.slice(-200))); } catch { /* ignore */ }
}
...
async function flushOutbox(): Promise<void> {
  if (!reportingOn() || flushing) return;
  ...
  for (const row of rows) {
    if (await postJson(row) === 'failed') left.push(row);   // keep it for the next attempt
  }
...
/** Which takes have reached the teacher, so nothing is uploaded twice. */
const sentTakes = new Set<string>();

/** Uploads one recording. Returns false if it did not get through. */
async function sendTake(unitTitle: string, task: string): Promise<boolean> {
  if (!reportingOn() || !state.student) return false;
  const blob = takes.get(task);
...
/** Uploads the session's takes one by one; failures are reported, not hidden. */
async function sendTakes(unitTitle: string, onProgress: (msg: string) => void): Promise<void> {
  if (!reportingOn()) { onProgress('Sending is switched off for this site.'); return; }
  if (!state.student) { onProgress('Add your name first.'); return; }
  const keys = [...takes.keys()];
  if (!keys.length) { onProgress('Record something first.'); return; }

  const pending = keys.filter((k) => !sentTakes.has(k));
  if (!pending.length) { onProgress(`All ${keys.length} recordings are already with your teacher.`); return; }

  let sent = 0, failed = 0;
  for (const k of pending) {
    onProgress(`Sending ${sent + failed + 1} of ${pending.length}…`);
    if (await sendTake(unitTitle, k)) sent++; else failed++;
  }
  onProgress(failed
    ? `${sent} sent, ${failed} could not go. Check the connection and press the button again.`
    : `All ${sent} recordings sent to your teacher.`);
}

function paintOutbox(): void {
...
function report(unit: Unit | null, activity: string, score: number, correct: number,
                total: number, misses: Miss[] = []): void {
  if (!reportingOn() || !state.student) return;
...
const KOKORO_VOICES: Array<{ id: string; name: string }> = [
...
/**
 * Kokoro clips kept in IndexedDB.
 *
 * Generation is the slow part, and the vocabulary is small and fixed, so a word
 * only ever needs generating once per device instead of once per session. The
 * raw samples are stored, not an encoded file, so playback needs no decoding.
 */
const ClipStore = {
  db: null as IDBDatabase | null,
  broken: false,

  /** Clips made by the WebGPU build were corrupt; that store is discarded. */
  purgeOld(): void {
    try { indexedDB.deleteDatabase('consonant-lab-audio'); } catch { /* nothing to bin */ }
  },

  open(): Promise<IDBDatabase | null> {
    if (this.db) return Promise.resolve(this.db);
    if (this.broken || typeof indexedDB === 'undefined') return Promise.resolve(null);
    return new Promise((resolve) => {
      let req: IDBOpenDBRequest;
      try { req = indexedDB.open('consonant-lab-audio-v2', 1); }
      catch { this.broken = true; return resolve(null); }
      req.onupgradeneeded = () => {
        if (!req.result.objectStoreNames.contains('clips')) req.result.createObjectStore('clips');
      };
      req.onsuccess = () => { this.db = req.result; resolve(this.db); };
      req.onerror = () => { this.broken = true; resolve(null); };
      // A blocked or hung open must not stall playback for ever.
      setTimeout(() => resolve(this.db), 2500);
    });
  },

  async get(key: string): Promise<{ data: Float32Array; sr: number } | null> {
  ...
  async put(key: string, value: { data: Float32Array; sr: number }): Promise<void> {
    const db = await this.open();
    if (!db) return;
    try { db.transaction('clips', 'readwrite').objectStore('clips').put(value, key); }
    catch { /* quota or private mode — the memory cache still helps */ }
  },
};

interface BankSet { id: string; label: string; engine?: string; voice?: string; }

/**
 * Pre-generated audio files listed in audio/manifest.json.
 *
 * This is the engine the classroom should actually use: nothing is computed at
 * run time, so playback is instant, every computer hears exactly the same
 * recording, and there is no model to download. Generate the files once with
 * tools/make-audio.mjs and commit them.
 */
class ClipEngine {
  sets: BankSet[] = [];
  clips: Record<string, string> = {};
  /** False until the manifest fetch has settled, either way. */
  loaded = false;
  private cache = new Map<string, HTMLAudioElement>();
  private current: HTMLAudioElement | null = null;

  /** Prefix the clip paths are relative to — set from wherever the manifest was found. */
  private base = 'audio/';

  async load(): Promise<void> {
    // GitHub's web uploader flattens folders, so a manifest meant for audio/
    // often lands in the root. Try both rather than silently losing the bank.
    const places = ['audio/manifest.json', 'manifest.json'];
    // Remember which one worked so later loads do not repeat the failed probe
    // and litter the console with a 404 on every page view.
    try {
      const known = localStorage.getItem('consonant-lab-manifest');
      if (known && places.includes(known)) places.sort((a) => (a === known ? -1 : 1));
    } catch { /* storage blocked — just probe in the default order */ }

    try {
      let m: any = null;
      for (const where of places) {
        try {
          const res = await fetch(where, { cache: 'no-cache' });
          if (!res.ok) continue;
          m = await res.json();
          this.base = where.includes('/') ? where.replace(/manifest\.json$/, '') : '';
          try { localStorage.setItem('consonant-lab-manifest', where); } catch { /* ignore */ }
          break;
        } catch { /* try the next location */ }
      }
      if (!m) return;
      this.sets = Array.isArray(m.sets) ? m.sets : [];
      this.clips = m.clips || {};
      if (!state.settings.bankSet || !this.sets.some((s) => s.id === state.settings.bankSet)) {
        state.settings.bankSet = this.sets[0] ? this.sets[0].id : null;
      }
    } catch {
      /* no bank on the server — the other engines cover it */
    } finally {
      this.loaded = true;
    }
  }

  get available(): boolean { return Object.keys(this.clips).length > 0; }

  key(text: string): string { return String(text).trim().toLowerCase().replace(/\s+/g, ' '); }

  has(text: string): boolean { return !!this.clips[this.key(text)]; }

  /** Fraction of these lines the bank can actually speak. */
  coverage(texts: string[]): number {
    if (!texts.length) return 1;
    return texts.filter((t) => this.has(t)).length / texts.length;
  }

  private el(path: string): HTMLAudioElement {
    let a = this.cache.get(path);
    if (!a) {
      a = new Audio(this.base + path);
      a.preload = 'auto';
      if (this.cache.size > 120) {
        const oldest = this.cache.keys().next().value as string;
        this.cache.delete(oldest);
      }
      this.cache.set(path, a);
    }
    return a;
  }
  ...
  preload(texts: string[]): void {
    for (const t of texts) {
      const p = this.clips[this.key(t)];
      if (p) this.el(p).load();
    }
  }

  async say(text: string, rate: number): Promise<boolean> {
    const path = this.clips[this.key(text)];
    if (!path) return false;
    const a = this.el(path);
    try {
      a.currentTime = 0;
      // The files are recorded at a natural pace; this only trims or stretches.
      a.playbackRate = Math.min(2, Math.max(0.5, rate / 0.9));
      this.current = a;
  ...
  stop(): void {
    if (this.current) {
      try { this.current.pause(); } catch { /* already stopped */ }
      this.current = null;
    }
  }
}
...
class KokoroEngine {
  ...
  get ready(): boolean { return this.status === 'ready' && !!this.tts; }
  ...
  load(): Promise<void> {
    ...
    this.loading = (async () => {
      ...
      // Deliberately NOT WebGPU. The q8 weights come out numerically wrong on
      // the WebGPU path — the voice turns into garbled non-English. WebGPU
      // needs dtype fp32, which is a ~326 MB download, so WASM it is.
  ...
  private key(text: string, speed: number): string {
    return `${state.settings.kokoroVoice}|${speed.toFixed(2)}|${text}`;
  }

  /** Generate (or fetch from cache) without playing — used to prefetch. */
  async prepare(text: string, speed: number): Promise<AudioBuffer | null> {
    if (!this.ready) return null;
    const k = this.key(text, speed);
    const hit = this.cache.get(k);
    if (hit) return hit;

    const stored = await ClipStore.get(k);
    if (stored) {
      const buf = this.audioCtx().createBuffer(1, stored.data.length, stored.sr);
      buf.getChannelData(0).set(stored.data);
      this.cache.set(k, buf);
      return buf;
    }

    try {
      const raw: any = await this.tts.generate(text, {
        voice: state.settings.kokoroVoice,
        speed,
      });
      const data: Float32Array = raw.audio ?? raw.data;
      const sr: number = raw.sampling_rate ?? raw.samplingRate ?? 24000;
      const buf = this.audioCtx().createBuffer(1, data.length, sr);
      buf.getChannelData(0).set(data);
      if (this.cache.size > 220) this.cache.delete(this.cache.keys().next().value as string);
      this.cache.set(k, buf);
      void ClipStore.put(k, { data: new Float32Array(data), sr });
      return buf;
    } catch {
      return null;
    }
  ...
  async say(text: string, speed: number): Promise<boolean> {
...
interface StudioVoice { id: string; name: string; kind: string; }

/**
 * Live speech from VoiceStudio (github.com/debpalash/VoiceStudio) running on
 * a machine the browser can reach. It speaks OpenAI's audio protocol, so one
 * POST returns the finished audio — no queue, no model download in the page.
 *
 * Two things decide whether a browser can talk to it, and both are the
 * teacher's to set, not ours:
 *
 *   CORS    VoiceStudio only answers origins listed in
 *           OMNIVOICE_ALLOWED_ORIGINS. A site on github.io is not there by
 *           default, so it has to be added or every request fails.
 *   Mixed   An https page may call http://127.0.0.1 (browsers treat loopback
 *   content as trustworthy) but NOT http://192.168.x.x. Reaching another
 *           machine therefore needs an https tunnel, not a bare LAN address.
 *
 * Generated audio is cached in memory and in IndexedDB, so a word costs one
 * request the first time and nothing after that.
 */
class StudioEngine {
  private ctx: AudioContext | null = null;
  private cache = new Map<string, AudioBuffer>();
  ...
  /** The address that answered, once one has, and which protocol it spoke. */
  private found = '';
  proto: 'voicestudio' | 'piper' = 'voicestudio';

  base(): string {
    const set = (state.settings.studioUrl || '').trim().replace(/\/+$/, '');
    return set || this.found || DEFAULT_STUDIO_URL;
  }

  /**
   * Addresses to try when the teacher has not named one, best first.
   *
   * Same origin comes first and is what makes this need no setting up: serve
   * the page from the voice server itself (Piper's img/ folder, VoiceStudio's
   * frontend/dist) and page and API share an origin — nothing to type, and no
   * CORS at all, which matters because Piper sends no CORS headers and cannot
   * be reached cross-origin by any means. Then each app's own port, for a
   * teacher working on the machine the server runs on.
   */
  private candidates(): string[] {
    const list: string[] = [];
    if (location.origin && /^https?:/.test(location.origin)) list.push(location.origin);
    for (const u of [DEFAULT_STUDIO_URL, DEFAULT_PIPER_URL]) if (!list.includes(u)) list.push(u);
    return list;
  }

  /**
   * Free ngrok answers a browser with an interstitial warning page instead of
   * the real response, which would arrive here as HTML where JSON was
   * expected. This header opts out of it. It is only added on a same-origin
   * call — the case where the page itself came through the tunnel — because a
   * custom header on a cross-origin request forces a preflight for nothing.
   */
  private headers(url: string, extra: Record<string, string> = {}): Record<string, string> {
    return url === location.origin ? { 'ngrok-skip-browser-warning': 'true', ...extra } : extra;
  }

  /**
   * One address, one answer, whichever of the two servers is behind it.
   *
   * VoiceStudio answers OpenAI's /v1/audio/voices; Piper answers its own
   * /voices. Asking for both and keeping whichever replies means the teacher
   * never has to say which program they are running.
   */
  private async ask(url: string): Promise<StudioVoice[]> {
    try {
      const voices = await this.askVoiceStudio(url);
      this.proto = 'voicestudio';
      return voices;
    } catch (e) {
      const voices = await this.askPiper(url).catch(() => { throw e; });
      this.proto = 'piper';
      return voices;
    }
  }

  /** Piper: GET /voices answers { "<voice id>": <config>, … }. */
  private async askPiper(url: string): Promise<StudioVoice[]> {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), 8000);
    try {
      const res = await fetch(`${url}/voices`, { signal: ctrl.signal, headers: this.headers(url) });
      if (!res.ok) throw new Error(`the server answered ${res.status}`);
      if (!/json/i.test(res.headers.get('content-type') || '')) {
        throw new Error('something other than a voice server answered at that address');
      }
      const body = await res.json();
      const out: StudioVoice[] = [];
      for (const [id, cfg] of Object.entries<any>(body || {})) {
        const lang = cfg?.language?.name_english || '';
        // One Piper file can hold many speakers, which are separate voices to
        // anyone choosing one.
        const speakers = Object.keys(cfg?.speaker_id_map || {});
        if (speakers.length > 1) {
          for (const sp of speakers) out.push({ id: `${id}#${sp}`, name: `${id} · ${sp}`, kind: lang });
        } else {
          out.push({ id, name: id, kind: lang });
        }
      }
      if (!out.length) throw new Error('it has no voices yet — download one with piper.download_voices');
      return out;
    } finally {
      clearTimeout(timer);
    }
  ...
  /** VoiceStudio: the OpenAI-compatible listing. */
  private async askVoiceStudio(url: string): Promise<StudioVoice[]> {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), 8000);
    try {
      const res = await fetch(`${url}/v1/audio/voices`, {
        signal: ctrl.signal,
      ...
      if (!/json/i.test(ctype)) {
        // A tunnel's own warning or login page, not a voice server.
        throw new Error('something other than a voice server answered at that address');
      }
      const body = await res.json();
      const voices = (body.voices || []).map((v: any) => ({
        ...
        kind: v.type === 'openai_alias' ? 'built-in' : (v.language || 'cloned'),
      }));
      if (!voices.length) throw new Error('it has no voices yet — create one in the app');
      return voices;
    } finally {
      clearTimeout(timer);
    }
  ...
  /** Ask the server which voices it has. Doubles as the connection test. */
  probe(force = false): Promise<void> {
    if (this.probing && !force) return this.probing;
    this.status = 'loading';
    this.error = '';
    this.onchange?.();

    this.probing = (async () => {
      const typed = (state.settings.studioUrl || '').trim().replace(/\/+$/, '');
      const tries = typed ? [typed] : this.candidates();
      let last: unknown = new Error('nothing to try');

      try {
        for (const url of tries) {
          try {
            this.voices = await this.ask(url);
            this.found = url;
            break;
          } catch (e) {
            last = e;
        ...
        if (!this.voices.length) throw last;

        if (!this.voices.some((v) => v.id === state.settings.studioVoice)) {
          const preferred = this.voices.find((v) => v.kind !== 'built-in') || this.voices[0];
          state.settings.studioVoice = preferred.id;
          saveState();
        }
        this.status = 'ready';
      } catch (e: unknown) {
  ...
  /** Turn the browser's deliberately vague failures into something actionable. */
  private explain(e: unknown): string {
    const msg = e instanceof Error ? e.message : String(e);
    if (/abort/i.test(msg)) return 'VoiceStudio did not answer in time. Is it running?';
    if (/^the server answered/.test(msg) || /no voices yet/.test(msg)
        || /other than VoiceStudio/.test(msg)) return msg;
    const typed = (state.settings.studioUrl || '').trim();
    const url = this.base();
    if (/^http:/.test(url) && location.protocol === 'https:' && !/\/\/(127\.0\.0\.1|localhost)\b/.test(url)) {
      return `This page is https, so the browser blocks plain http to ${url}. `
        + 'Use an https address (a tunnel), or run VoiceStudio on this same computer.';
    }
    if (!typed) {
      // Nothing was typed, so naming one address would be misleading — say
      // what was actually tried, and give the shortest route to fixing it.
      return `Looked for VoiceStudio at ${this.candidates().join(' and ')} and found nothing. `
        + 'Open it on this computer, or publish it with "ngrok http 3900" and open this page '
        + 'through that same address.';
    }
    return `Could not reach VoiceStudio at ${url}. Check that it is running, and that `
      + `OMNIVOICE_ALLOWED_ORIGINS includes ${location.origin}`;
  }

  private key(text: string, speed: number): string {
    return `vs|${this.proto}|${this.base()}|${state.settings.studioModel}|${state.settings.studioVoice}|${speed.toFixed(2)}|${text}`;
  }
  ...
  async prepare(text: string, speed: number): Promise<AudioBuffer | null> {
    ...
    // Prefetch and playback often want the same clip at once; without this the
    // same word would be generated twice.
    ...
    const job = (async (): Promise<AudioBuffer | null> => {
      ...
      try {
        const url = this.base();
        const res = this.proto === 'piper'
          ? await this.fetchPiper(url, text, speed)
          : await this.fetchVoiceStudio(url, text, speed);
        if (!res.ok) throw new Error(`the server answered ${res.status}`);
        const buf = await this.audioCtx().decodeAudioData(await res.arrayBuffer());

        if (this.cache.size > 220) this.cache.delete(this.cache.keys().next().value as string);
        this.cache.set(k, buf);
        void ClipStore.put(k, { data: new Float32Array(buf.getChannelData(0)), sr: buf.sampleRate });
        return buf;
      } catch (e: unknown) {
        // A failure mid-lesson must surface, or the app just goes quiet.
        this.status = 'error';
        this.error = this.explain(e);
        this.onchange?.();
        return null;
  ...
  private fetchVoiceStudio(url: string, text: string, speed: number): Promise<Response> {
    return fetch(`${url}/v1/audio/speech`, {
      method: 'POST',
      headers: this.headers(url, { 'Content-Type': 'application/json' }),
      body: JSON.stringify({
        model: state.settings.studioModel,
        input: text,
        voice: state.settings.studioVoice,
        response_format: 'mp3',
  ...
  /**
   * Piper takes length_scale, which is duration rather than rate — the
   * reciprocal of speed. Half speed is twice the length, so passing the speed
   * straight through would make every slow replay come out fast instead.
   */
  private fetchPiper(url: string, text: string, speed: number): Promise<Response> {
    const [model, speaker] = String(state.settings.studioVoice).split('#');
    const body: Record<string, unknown> = {
      text,
      length_scale: 1 / Math.min(4, Math.max(0.25, speed)),
    };
    if (model) body.voice = model;
    if (speaker) body.speaker = speaker;
    return fetch(`${url}/synthesize`, {
      method: 'POST',
      headers: this.headers(url, { 'Content-Type': 'application/json' }),
...
class DeviceEngine {
  ...
  private preferred(): SpeechSynthesisVoice {
    const score = (v: SpeechSynthesisVoice) => {
      ...
      if (/samantha|google us|aria|jenny|zira|alex|daniel/i.test(v.name)) s += 3;
      return s;
    };
    return this.voices.slice().sort((a, b) => score(b) - score(a))[0];
  }

  list(): SpeechSynthesisVoice[] { return this.voices; }
  get available(): boolean { return this.voices.length > 0; }

  stop(): void { if (this.supported) speechSynthesis.cancel(); }
  ...
  say(text: string, rate: number): Promise<void> {
    return new Promise((resolve) => {
      ...
      poll = setInterval(() => {
        if (speechSynthesis.speaking || speechSynthesis.pending) { started = true; return; }
        if (started) finish();
...
class Speaker {
  bank = new ClipEngine();
  device = new DeviceEngine();
  kokoro = new KokoroEngine();
  studio = new StudioEngine();
  onchange: (() => void) | null = null;
  private busy = false;
  ...
  async init(): Promise<void> {
    ...
    this.studio.onchange = () => this.onchange?.();
    this.device.init();
    await this.bank.load();
    // Fall back off the bank if the files are not there yet.
    if (state.settings.engine === 'bank' && this.bank.loaded && !this.bank.available) {
      state.settings.engine = 'device';
    }
    // The first time real clips do show up, move everyone onto them. Students
    // who used the site before the bank existed were put on the device voice by
    // the line above, and without this they would keep that robotic voice for
    // good. It runs once; picking another voice afterwards still sticks.
    if (this.bank.available && !state.settings.bankSeen) {
      state.settings.bankSeen = true;
      state.settings.engine = 'bank';
      saveState();
    }
    if (state.settings.engine === 'kokoro') void this.kokoro.load().catch(() => { /* reported in UI */ });
    // Contacting VoiceStudio is cheap, so the voice list is ready the moment
    // the teacher opens Settings rather than after another round trip.
    if (state.settings.engine === 'studio') void this.studio.probe();
    this.onchange?.();
  }

  get engine(): EngineName {
    if (state.settings.engine === 'bank' && this.bank.available) return 'bank';
    if (state.settings.engine === 'studio' && this.studio.ready) return 'studio';
    return state.settings.engine === 'kokoro' && this.kokoro.ready ? 'kokoro' : 'device';
  }

  stop(): void { this.bank.stop(); this.device.stop(); this.kokoro.stop(); this.studio.stop(); }

  async say(text: string, rateMul = 1): Promise<void> {
    const rate = Math.min(1.6, Math.max(0.4, state.settings.rate * rateMul));
    this.busy = true;
    document.body.classList.add('speaking');
    try {
      // A recorded clip always wins: it is instant and identical everywhere.
      if (state.settings.engine === 'bank' && this.bank.has(text)) {
        if (await this.bank.say(text, rate)) return;
      }
      if (state.settings.engine === 'studio') {
        // Not ready usually means the page has just loaded; ask once rather
        // than dropping the student onto the robotic voice for the first word.
        if (!this.studio.ready && this.studio.status !== 'error') await this.studio.probe();
        if (this.studio.ready && await this.studio.say(text, rate)) return;
      }
  ...
  /**
   * Warm the cache for what is about to be said. This is what hides the wait
   * on a generating engine: the next question is fetched while the student is
   * still answering this one, so by the time it plays it is already local.
   */
  prefetch(texts: string[], rateMul = 1): void {
    if (state.settings.engine === 'bank') { this.bank.preload(texts); return; }
    if (this.busy) return;
    const rate = Math.min(1.6, Math.max(0.4, state.settings.rate * rateMul));

    if (state.settings.engine === 'studio' && this.studio.ready) {
      void (async () => { for (const t of texts) await this.studio.prepare(t, rate); })();
      return;
    }
    if (state.settings.engine !== 'kokoro' || !this.kokoro.ready) return;
    void (async () => { for (const t of texts) await this.kokoro.prepare(t, rate); })();
  }

  async sequence(items: string[], gapMs = 380, rateMul = 1): Promise<void> {
...
/* ============================================================================
   7b. MICROPHONE — the Speaking Studio
   ==========================================================================
   Recordings are Blobs, which do not survive JSON.stringify, so they live in
   memory for the session only. Students send them to the teacher before they
   leave the page; a reload clears them, and the UI says so.
   ========================================================================== */
...
class Recorder {
  ...
  get recording(): boolean { return !!this.mr && this.mr.state === 'recording'; }
  ...
  async start(onLevel: (pct: number) => void, onStop: (b: Blob) => void): Promise<boolean> {
    ...
    this.mr.onstop = () => {
      onStop(new Blob(this.chunks, { type: this.mr?.mimeType || 'audio/webm' }));
    ...
    // Hard stop so a forgotten recording cannot grow without limit.
    this.timer = setTimeout(() => this.stop(), MAX_CLIP_SECONDS * 1000) as unknown as number;
    return true;
  }

  stop(): void { if (this.recording) this.mr!.stop(); }
}

const recorder = new Recorder();

/** word/sentence -> the student's own recording, this session only */
...
function discriminationSets(unit: Unit, n: number): string[][] {
  const sets = shuffle(unit.triples).slice(0, n);
  if (sets.length < n) {
    for (const p of shuffle(unit.pairs)) {
      if (sets.length >= n) break;
      sets.push([p.a, p.b]);
    }
  }
  return sets;
...
function buildQuestions(unit: Unit, kind: ActivityKind): Question[] {
  switch (kind) {
    case 'pairs':
    ...
    case 'pairsx':
      return discriminationSets(unit, 15).map((set) => {
        const opts = shuffle(set);
        const heard = rand(opts);
        const carrier = carrierFor(heard);
        return {
...
let viewClick: ((e: MouseEvent) => void) | null = null;

/**
 * #view survives every navigation — only its innerHTML changes — so a listener
 * attached to it by one screen is still attached when the next one arrives.
 * Screens register theirs here and setView takes them off again.
 */
function onViewClick(fn: (e: MouseEvent) => void): void {
  if (viewClick) app().removeEventListener('click', viewClick);
  viewClick = fn;
...
function setView(html: string): void {
  ...
  if (viewClick) { app().removeEventListener('click', viewClick); viewClick = null; }
  const v = app();
  v.innerHTML = html;
...
function viewIdentity(): void {
    ...
    <section class="card">
      <div class="idform">
        <div class="field">
          ...
          <input type="text" id="sname" autocomplete="name" placeholder="Name and surname" maxlength="60">
        </div>
        <p class="iderr" id="iderr" hidden role="alert">Please type your name.</p>
        <button class="btn btn-solid lg" id="idgo">Start &rarr;</button>
        <p class="help">Write it the same way every time so all your results stay together.</p>
      </div>
    </section>
  `);
  ...
  const go = () => {
    ...
    if (!name) { $('#iderr').hidden = false; return; }
    state.student = { name };
    saveState();
    speaker.onchange?.();   // refresh the Audio panel with the new name
    viewHome();
  };
...
function viewReview(unit: Unit, idx = 0): void {
...
/* ---------- Speaking Studio: hear the model, record yourself, compare ------ */

function studioItems(unit: Unit): string[] {
  // The triples carry the contrast the unit is about, so they are what is
  // worth saying out loud. Fall back to the pairs for anything missing.
  const out: string[] = [];
  for (const t of unit.triples) out.push(...t);
  for (const p of unit.pairs) out.push(p.a, p.b);
  return [...new Set(out)].slice(0, 12);
}

function viewStudio(unit: Unit): void {
  const items = studioItems(unit);

  setView(`
    <nav class="crumb"><button class="link" data-go="drills" data-unit="${unit.id}">&larr; Drills</button>
      <span>Unit ${unit.num} &middot; Speaking Studio</span></nav>

    <header class="drill-head">
      <p class="eyebrow">${unit.book} &middot; Say it yourself</p>
      <h1>Speaking Studio</h1>
      <p class="lede">Play the model, record your own version, then play them one after the other
      and listen for the difference. Recordings live in this page only &mdash; if you reload, they are gone,
      so send them before you leave.</p>
      ${recorder.supported ? '' : `<p class="banner">Your browser will not let this page use the microphone.
        Try Chrome, Edge or Safari, and open the site over <b>https</b>.</p>`}
    </header>

    <div class="studio">
      ${items.map((w, i) => `
        <article class="take" data-word="${escapeHtml(w)}" style="--i:${i}">
          <div class="take-word">${escapeHtml(w)}<span class="tick" aria-hidden="true"></span></div>
          <div class="take-row">
            <button class="mini" data-say="${escapeHtml(w)}">model</button>
            <button class="mini rec" data-rec="${escapeHtml(w)}" ${recorder.supported ? '' : 'disabled'}>record</button>
            <button class="mini" data-play-take="${escapeHtml(w)}" disabled>yours</button>
            <button class="mini" data-ab="${escapeHtml(w)}" disabled>compare</button>
          </div>
    ...
    <div class="pager">
      <span class="hint" id="studio-msg">${
        takes.size
          ? `${sentTakes.size} of ${takes.size} recordings sent to your teacher.`
          : 'Recordings are sent to your teacher as soon as you make them.'
      }</span>
      ${reportingOn() ? '<button class="btn btn-solid" id="sendtakes">Send my recordings</button>' : ''}
    </div>
  `);

  const msg = (t: string) => {
    const el = document.getElementById('studio-msg');
    if (el) el.textContent = t;      // the student may have navigated away mid-send
  };

  const refresh = (word: string) => {
    const card = document.querySelector(`.take[data-word="${CSS.escape(word)}"]`);
    if (!card || !card.isConnected) return;
    const has = takes.has(word);
    card.classList.toggle('has', has);
    card.classList.toggle('sent', sentTakes.has(word));
    (card.querySelector('[data-play-take]') as HTMLButtonElement).disabled = !has;
    (card.querySelector('[data-ab]') as HTMLButtonElement).disabled = !has;
    (card.querySelector('[data-rec]') as HTMLButtonElement).textContent = has ? 're-record' : 'record';
  };
  items.forEach(refresh);

  const playTake = (word: string): Promise<void> => new Promise((resolve) => {
    const blob = takes.get(word);
  ...
  onViewClick(async (e) => {
    ...
    const rec = el.closest<HTMLElement>('[data-rec]');
    if (rec) {
      const word = rec.dataset.rec!;
      const card = rec.closest('.take')!;
      const bar = card.querySelector('.level i') as HTMLElement;
      ...
      const ok = await recorder.start(
        ...
        async (blob) => {
          takes.set(word, blob);
          sentTakes.delete(word);          // a re-record must go again
          card.classList.remove('recording');
          bar.style.width = '0%';
          refresh(word);
          // Send it straight away. Students should not have to remember a
          // button, and a reload wipes the takes.
          if (reportingOn() && state.student) {
            card.classList.add('sending');
            msg(`Sending “${word}” to your teacher…`);
            const ok = await sendTake(unit.title, word);
            // The upload is async: by now the student may have left the studio,
            // so every node captured above can be detached.
            if (card.isConnected) card.classList.remove('sending');
            refresh(word);
            msg(ok
              ? `“${word}” sent. ${sentTakes.size} of ${takes.size} recordings are with your teacher.`
              : `“${word}” could not be sent. Press “Send my recordings” to try again.`);
          } else {
            msg(`${takes.size} recording${takes.size === 1 ? '' : 's'} in this session.`);
          }
        });
      if (!ok) {
        card.classList.remove('recording');
        refresh(word);
        msg('The microphone was refused. Allow it in the address bar and try again.');
      }
      return;
    }

    const play = el.closest<HTMLElement>('[data-play-take]');
    if (play) { speaker.stop(); void playTake(play.dataset.playTake!); return; }

    const ab = el.closest<HTMLElement>('[data-ab]');
    if (ab) {
      const word = ab.dataset.ab!;
      speaker.stop();
      await speaker.say(word);
  ...
  const send = document.getElementById('sendtakes');
  if (send) send.addEventListener('click', async () => {
    (send as HTMLButtonElement).disabled = true;
    await sendTakes(unit.title, msg);
...
function viewDrills(unit: Unit): void {
  ...
  setView(`
    ...
    <article class="studio-band">
      <div>
        <p class="eyebrow">Say it yourself</p>
        <h2>Speaking Studio</h2>
        <p class="lede sm">Hear the model, record your own version, compare the two &mdash;
        and send the clips to your teacher.</p>
      </div>
      <button class="btn btn-solid lg" data-go="studio" data-unit="${unit.id}">Open the studio</button>
    </article>
...
/** One wrong answer, in the terms the teacher needs: what played, what it
 *  actually was, and what the student picked instead. */
interface Miss { heard: string; correct: string; chosen: string; }
...
interface Session {
  ...
  answered: boolean;
  unit: Unit | null;
  activity: ActivityKind | 'exam';
  title: string;
}

let session: Session | null = null;

function startQuiz(unit: Unit | null, activity: ActivityKind | 'exam'): void {
  const questions = unit ? buildQuestions(unit, activity as ActivityKind) : buildExam();
  session = {
    questions, i: 0, correct: 0, misses: [], answered: false, unit, activity,
...
function renderQuestion(): void {
  ...
  const useReplay = (rateMul?: number) => {
    if (session?.answered) { play(rateMul); return; }
    if (plays >= q.maxReplays) return;
    plays++;
    const left = q.maxReplays - plays;
    $('#replays').textContent = `${left} replay${left === 1 ? '' : 's'} left`;
    if (left === 0) {
      playBtn.disabled = true;
      const slow = document.getElementById('slow') as HTMLButtonElement | null;
  ...
  if (slowBtn) slowBtn.addEventListener('click', () => useReplay(0.6));
  ...
  keyHandler = (e: KeyboardEvent) => {
    ...
    if (e.key.toLowerCase() === 'r') { useReplay(); return; }
  ...
  $('#options').addEventListener('click', (e) => {
    const btn = (e.target as HTMLElement).closest('[data-pick]') as HTMLElement | null;
...
function answer(choice: number): void {
  ...
  Array.from(document.querySelectorAll<HTMLElement>('.opt')).forEach((el, k) => {
...
function finishQuiz(): void {
  ...
  saveState();
  report(unit, activity, score, correct, questions.length, session.misses);
...
const unitById = (id: string): Unit => UNITS.find((u) => u.id === id)!;

function route(go: string, unitId?: string): void {
  switch (go) {
...
function wireGlobalClicks(): void {
  document.addEventListener('click', (e) => {
    ...
    const say = el.closest<HTMLElement>('[data-say]');
    if (say) {
    ...
    const run = el.closest<HTMLElement>('[data-run]');
    if (run && !run.hasAttribute('disabled')) {
      startQuiz(unitById(run.dataset.unit!), run.dataset.run as ActivityKind);
      return;
    }

    const rev = el.closest<HTMLElement>('[data-review]');
    if (rev && !rev.hasAttribute('disabled')) {
      viewReview(unitById($('#view').dataset.unit!), Number(rev.dataset.review));
      return;
    }

    const fin = el.closest<HTMLElement>('[data-finish-review]');
    if (fin) {
...
function wireSettings(): void {
  ...
  const kvoice = $<HTMLSelectElement>('#kvoice');
  const theme = $<HTMLButtonElement>('#theme');
  const panel = $('#settings');
  const toggle = $('#settings-toggle');
  const engineRadios = Array.from(document.querySelectorAll<HTMLInputElement>('input[name="engine"]'));
  ...
  const bankset = $<HTMLSelectElement>('#bankset');
  bankset.addEventListener('change', () => {
    state.settings.bankSet = bankset.value;
    saveState();
    void speaker.say('This is the voice you will hear.');
  });

  /* ---- VoiceStudio ---- */
  const studioVoice = $<HTMLSelectElement>('#studio-voice');
  const studioModel = $<HTMLSelectElement>('#studio-model');
  const studioUrl = $<HTMLInputElement>('#studio-url');

  studioVoice.addEventListener('change', () => {
    state.settings.studioVoice = studioVoice.value;
    saveState();
    void speaker.say('This is the voice you will hear.');
  });
  studioModel.addEventListener('change', () => {
    state.settings.studioModel = studioModel.value;
    saveState();
    // Engines differ in which voices they accept, so re-ask rather than assume.
    void speaker.studio.probe(true);
  });
  // Committed on blur or Enter, not per keystroke — probing every character
  // typed into an address would fire a request per letter.
  const commitUrl = () => {
    const next = studioUrl.value.trim() || DEFAULT_STUDIO_URL;
    if (next === state.settings.studioUrl) return;
    state.settings.studioUrl = next;
    saveState();
    void speaker.studio.probe(true);
  };
  studioUrl.addEventListener('change', commitUrl);
  studioUrl.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); commitUrl(); } });

  $('#studio-test').addEventListener('click', () => { commitUrl(); void speaker.studio.probe(true); });
  $('#studio-preview').addEventListener('click', () => {
    void speaker.say('This is the voice you will hear. Think, these, ship, chip.');
  });

  const paint = () => {
    // Who is signed in
    const who = $('#whoami-field');
    who.hidden = !(reportingOn() && state.student);
    if (state.student) $('#whoami').textContent = state.student.name;
    paintOutbox();

    // Recorded clip bank
    const bank = speaker.bank;
    // Keep the option on screen but disabled, so it is obvious the feature
    // exists and obvious why it cannot be picked yet.
    const bankRadio = $<HTMLInputElement>('input[name="engine"][value="bank"]');
    bankRadio.disabled = !bank.available;
    $('#engine-bank-wrap').classList.toggle('off', !bank.available);
    if (!bank.available) {
      $('#bank-note').innerHTML = bank.loaded
        ? `Not available yet &mdash; the audio files have not been made.
           <br>Start Piper with
           <code>python3 -m piper.http_server -m en_US-lessac-medium</code>,
           then in the project folder: <code>npm install</code>,
           <code>npm run voices</code> to see them, then
           <code>npm run audio -- --voice "&lt;name&gt;"</code>, and commit the
           <code>audio</code> folder it creates.`
        : 'Looking for the clips…';
    }
    if (bank.available) {
      const s = bank.sets.find((x) => x.id === state.settings.bankSet) || bank.sets[0];
      $('#bank-note').textContent = s && s.voice
        ? `${s.voice}, recorded ahead of time. Plays instantly and sounds the same on every computer.`
        : 'Ready-made audio files. Instant, and identical on every computer.';
      bankset.innerHTML = bank.sets
        .map((x) => `<option value="${escapeHtml(x.id)}" ${x.id === state.settings.bankSet ? 'selected' : ''}>${escapeHtml(x.label)}</option>`)
        .join('');
      // Say plainly how much of the material the bank can actually speak.
      const needed: string[] = [];
      for (const u of UNITS) {
        for (const t of u.triples) needed.push(...t);
        for (const p of u.pairs) needed.push(p.a, p.b);
        for (const s2 of u.sentences) needed.push(plain(s2.a), plain(s2.b));
      }
      const cov = Math.round(bank.coverage([...new Set(needed)]) * 100);
      $('#bank-cov').textContent = cov >= 100
        ? 'Every word and sentence in the app has a recording.'
        : `${cov}% of the material has a recording; the rest falls back to the device voice.`;
    } else if (state.settings.engine === 'bank' && bank.loaded) {
      // Only give up on the bank once the manifest has actually been checked;
      // doing it earlier flips every first-time visitor onto the device voice.
      state.settings.engine = 'device';
    }
    $('#bank-opts').hidden = state.settings.engine !== 'bank' || !bank.available;
    ...
    // VoiceStudio
    const st = speaker.studio;
    const useS = state.settings.engine === 'studio';
    $('#studio-opts').hidden = !useS;
    studioUrl.value = state.settings.studioUrl || '';
    studioModel.value = state.settings.studioModel;
    if (st.voices.length) {
      studioVoice.disabled = false;
      studioVoice.innerHTML = st.voices
        .map((v) => `<option value="${escapeHtml(v.id)}" ${v.id === state.settings.studioVoice ? 'selected' : ''}>${escapeHtml(v.name)} — ${escapeHtml(v.kind)}</option>`)
        .join('');
    } else {
      studioVoice.disabled = true;
      studioVoice.innerHTML = `<option>${st.status === 'loading' ? 'Asking VoiceStudio…' : 'No voices yet'}</option>`;
    }
    if (useS) {
      // The engine list is VoiceStudio's; Piper picks its model from the voice.
      const isPiper = st.proto === 'piper' && st.status === 'ready';
      studioModel.hidden = isPiper;
      $('#studio-model-label').hidden = isPiper;

      const out = $('#studio-out');
      if (st.status === 'error') { out.hidden = false; out.className = 'help connout bad'; out.textContent = st.error; }
      else if (st.status === 'ready') {
        out.hidden = false;
        out.className = 'help connout good';
        out.textContent = `${st.proto === 'piper' ? 'Piper' : 'VoiceStudio'} at ${st.base()}`
          + ` — ${st.voices.length} voice${st.voices.length === 1 ? '' : 's'} available.`;
      } else if (st.status === 'loading') {
        out.hidden = false; out.className = 'help connout'; out.textContent = 'Contacting VoiceStudio…';
      }
    }

    // Engine panels
    const useK = state.settings.engine === 'kokoro';
    $('#device-opts').hidden = useK || useS;
    ...
    if (!useK) { box.hidden = true; }
      ...
      if (k.status === 'loading') {
        ...
        box.innerHTML = `Kokoro is loaded. It generates on the CPU and takes a few seconds
          per word the first time it says it, then remembers it.
          <small>Recorded clips are still the better option for a classroom.</small>`;
  ...
  engineRadios.forEach((r) => r.addEventListener('change', () => {
    if (!r.checked) return;
    state.settings.engine = r.value as EngineName;
    saveState();
    paint();
    if (state.settings.engine === 'kokoro') {
    ...
    if (state.settings.engine === 'studio') {
      speaker.studio.probe(true).then(() => {
        paint();
        if (speaker.studio.ready) void speaker.say('VoiceStudio is connected.');
      });
    }
  }));
  ...
  // Wired once; the visible state is refreshed by paint(), because the student
  // signs in after this runs.
  ...
  // A real round-trip to the teacher's sheet, with the actual reason on screen.
  // Without this, a misconfigured deployment fails completely silently.
  $('#conn').addEventListener('click', async () => {
    const out = $('#conn-out');
    const btn = $<HTMLButtonElement>('#conn');
    if (!reportingOn()) { out.className = 'connout bad'; out.textContent = 'No endpoint is set in REPORT_URL.'; return; }
    btn.disabled = true;
    out.className = 'connout';
    out.textContent = 'Contacting the sheet…';
    const r = await postJson({
      kind: 'score',
      student: state.student ? state.student.name : 'Connection test',
      unit: 'TEST', unitTitle: 'Connection test',
      activity: 'test', activityName: 'Connection test',
      score: 0, correct: 0, total: 0, misses: [], engine: speaker.engine,
      at: new Date().toISOString(),
    });
    btn.disabled = false;
    if (r === 'ok') {
      out.className = 'connout good';
      out.innerHTML = 'Working. A row marked <b>TEST</b> should be in the Results tab now &mdash; delete it when you have seen it.';
    } else if (r === 'unconfirmed') {
      out.className = 'connout warn';
      out.innerHTML = `Sent, but the sheet did not reply, so it cannot be confirmed.
        Usually this means the deployment is running an older version.
        <br>In Apps Script: <b>Deploy &rarr; Manage deployments &rarr; pencil &rarr; Version: New version &rarr; Deploy</b>.`;
    } else {
      out.className = 'connout bad';
      out.innerHTML = `Nothing is reaching the sheet. The two usual causes:
        <br>1. <b>Deploy &rarr; Manage deployments</b>: <i>Who has access</i> must be
        <b>Anyone</b> &mdash; not "Anyone with Google account".
        <br>2. The Apps Script must be opened from the sheet itself
        (<b>Extensions &rarr; Apps Script</b>), not created separately, or it cannot find the sheet.`;
    }
  });

  $('#test').addEventListener('click', () =>
    void speaker.say('She sells sea shells. They think this. He mentioned the cats and the catch.'));
  ...
  theme.addEventListener('click', () => {
    const order: Array<SaveState['settings']['theme']> = ['auto', 'dark', 'light'];
...
function boot(): void {
  ...
  wireGlobalClicks();
  wireSettings();
  if (reportingOn() && !state.student) viewIdentity();
  else viewHome();
  void flushOutbox();
