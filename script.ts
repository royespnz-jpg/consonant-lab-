...
async function sendTakes(unitTitle: string, onProgress: (msg: string) => void): Promise<void> {
  ...
  let sent = 0, failed = 0;
  for (const k of keys) {
    onProgress(`Sending ${sent + failed + 1} of ${keys.length}…`);
    try {
      const data = await blobToBase64(takes.get(k)!);
      await fetch(REPORT_URL, {
        method: 'POST',
        mode: 'no-cors',
        headers: { 'Content-Type': 'text/plain;charset=utf-8' },
        body: JSON.stringify({
          ...
          unit: unitTitle,
          task: k,
          mime: takes.get(k)!.type || 'audio/webm',
          data,
  ...
  onProgress(failed
    ? `${sent} sent, ${failed} could not go. Check the connection and try again.`
...
function viewStudio(unit: Unit): void {
  ...
  setView(`
    ...
    <div class="studio">
      ${items.map((w, i) => `
        <article class="take" data-word="${escapeHtml(w)}" style="--i:${i}">
          <div class="take-word">${escapeHtml(w)}</div>
          <div class="take-row">
    ...
    <div class="pager">
      <span class="hint" id="studio-msg">${takes.size} recording${takes.size === 1 ? '' : 's'} in this session.</span>
