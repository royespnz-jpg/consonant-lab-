...
type EngineName = 'bank' | 'device' | 'kokoro';
...
const blankState = (): SaveState => ({
  ...
  units: {},
  exam: { best: 0, taken: 0 },
  settings: { engine: 'bank', bankSet: null, voiceURI: null, kokoroVoice: 'af_heart', rate: 0.9, theme: 'auto' },
});
...
class Speaker {
  ...
  stop(): void { this.bank.stop(); this.device.stop(); this.kokoro.stop(); }
  ...
  const paint = () => {
    ...
    if (!bank.available) {
      $('#bank-note').innerHTML = bank.loaded
        ? `Not available yet &mdash; the audio files have not been made.
           <br>On a computer with Node installed, in the project folder:
           <code>npm install</code> then <code>npm run audio</code>,
    ...
    if (bank.available) {
      ...
      $('#bank-note').textContent = s && s.voice
        ? `Recorded with ${s.engine || 'a generator'} (${s.voice}). Instant, and identical on every computer.`
