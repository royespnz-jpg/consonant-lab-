...
async function postJson(payload) {
    ...
    try {
        ...
        if (/<html/i.test(text) || /accounts\.google\.com/i.test(text))
            return 'failed';
        try {
...
const sentTakes = new Set();
/** Uploads one recording. Returns false if it did not get through. */
async function sendTake(unitTitle, task) {
...
async function sendTakes(unitTitle, onProgress) {
    if (!reportingOn()) {
        onProgress('Sending is switched off for this site.');
        return;
    }
    if (!state.student) {
        onProgress('Add your name first.');
        return;
    }
    const keys = [...takes.keys()];
    if (!keys.length) {
        onProgress('Record something first.');
        return;
    }
    const pending = keys.filter((k) => !sentTakes.has(k));
    if (!pending.length) {
        onProgress(`All ${keys.length} recordings are already with your teacher.`);
        return;
    }
    let sent = 0, failed = 0;
    for (const k of pending) {
        onProgress(`Sending ${sent + failed + 1} of ${pending.length}…`);
        if (await sendTake(unitTitle, k))
            sent++;
        else
            failed++;
    }
...
function report(unit, activity, score, correct, total, misses = []) {
    if (!reportingOn() || !state.student)
        return;
...
const ClipStore = {
    ...
    purgeOld() {
        try {
            indexedDB.deleteDatabase('consonant-lab-audio');
        }
        catch { /* nothing to bin */ }
    },
    open() {
        if (this.db)
            return Promise.resolve(this.db);
        if (this.broken || typeof indexedDB === 'undefined')
            return Promise.resolve(null);
        return new Promise((resolve) => {
            let req;
            try {
                req = indexedDB.open('consonant-lab-audio-v2', 1);
            }
            catch {
                this.broken = true;
                return resolve(null);
    ...
    async put(key, value) {
        ...
        if (!db)
            return;
        try {
            db.transaction('clips', 'readwrite').objectStore('clips').put(value, key);
        }
...
class ClipEngine {
    ...
    async load() {
        ...
        try {
            ...
            if (known && places.includes(known))
                places.sort((a) => (a === known ? -1 : 1));
        }
        ...
        try {
            ...
            for (const where of places) {
                try {
                    ...
                    try {
                        localStorage.setItem('consonant-lab-manifest', where);
                    }
                    catch { /* ignore */ }
                    break;
                }
    ...
    get available() { return Object.keys(this.clips).length > 0; }
    key(text) { return String(text).trim().toLowerCase().replace(/\s+/g, ' '); }
    has(text) { return !!this.clips[this.key(text)]; }
...
class StudioEngine {
    ...
    candidates() {
        ...
        for (const u of [DEFAULT_STUDIO_URL, DEFAULT_PIPER_URL])
            if (!list.includes(u))
                list.push(u);
        return list;
    }
    ...
    async askPiper(url) {
        ...
        try {
            ...
            for (const [id, cfg] of Object.entries(body || {})) {
                ...
                if (speakers.length > 1) {
                    for (const sp of speakers)
                        out.push({ id: `${id}#${sp}`, name: `${id} · ${sp}`, kind: lang });
                }
                else {
            ...
            if (!out.length)
                throw new Error('it has no voices yet — download one with piper.download_voices');
            return out;
        }
        finally {
            clearTimeout(timer);
    ...
    async askVoiceStudio(url) {
        ...
        try {
            ...
            if (!voices.length)
                throw new Error('it has no voices yet — create one in the app');
            return voices;
        }
        finally {
            clearTimeout(timer);
    ...
    probe(force = false) {
        ...
        this.probing = (async () => {
            ...
            let last = new Error('nothing to try');
            try {
    ...
    explain(e) {
        ...
        if (/abort/i.test(msg))
            return 'VoiceStudio did not answer in time. Is it running?';
        if (/^the server answered/.test(msg) || /no voices yet/.test(msg)
            || /other than VoiceStudio/.test(msg))
            return msg;
    ...
    fetchVoiceStudio(url, text, speed) {
...
class Speaker {
    ...
    async init() {
        ...
        if (state.settings.engine === 'studio')
            void this.studio.probe();
        this.onchange?.();
    }
    get engine() {
        if (state.settings.engine === 'bank' && this.bank.available)
            return 'bank';
        if (state.settings.engine === 'studio' && this.studio.ready)
            return 'studio';
        return state.settings.engine === 'kokoro' && this.kokoro.ready ? 'kokoro' : 'device';
    }
    stop() { this.bank.stop(); this.device.stop(); this.kokoro.stop(); this.studio.stop(); }
    async say(text, rateMul = 1) {
        ...
        try {
            ...
            if (state.settings.engine === 'bank' && this.bank.has(text)) {
                if (await this.bank.say(text, rate))
                    return;
            }
            if (state.settings.engine === 'studio') {
                ...
                if (!this.studio.ready && this.studio.status !== 'error')
                    await this.studio.probe();
                if (this.studio.ready && await this.studio.say(text, rate))
                    return;
            }
    ...
    prefetch(texts, rateMul = 1) {
        if (state.settings.engine === 'bank') {
            this.bank.preload(texts);
            return;
        }
        if (this.busy)
            return;
        ...
        if (state.settings.engine === 'studio' && this.studio.ready) {
            void (async () => { for (const t of texts)
                await this.studio.prepare(t, rate); })();
            return;
        }
        if (state.settings.engine !== 'kokoro' || !this.kokoro.ready)
            return;
...
class Recorder {
    ...
    async start(onLevel, onStop) {
        ...
        this.mr.ondataavailable = (e) => { if (e.data.size)
        ...
        const tick = () => {
            ...
            let peak = 0;
            for (const b of buf)
                peak = Math.max(peak, Math.abs(b - 128));
            onLevel(Math.min(100, (peak / 128) * 180));
            this.raf = requestAnimationFrame(tick);
        };
        tick();
        // Hard stop so a forgotten recording cannot grow without limit.
        this.timer = setTimeout(() => this.stop(), MAX_CLIP_SECONDS * 1000);
        return true;
    }
    stop() { if (this.recording)
        this.mr.stop(); }
}
 ...
 */
function onViewClick(fn) {
    if (viewClick)
        app().removeEventListener('click', viewClick);
    viewClick = fn;
...
function viewStudio(unit) {
    ...
    <div class="pager">
      <span class="hint" id="studio-msg">${takes.size
        ? `${sentTakes.size} of ${takes.size} recordings sent to your teacher.`
        : 'Recordings are sent to your teacher as soon as you make them.'}</span>
  ...
  `);
    const msg = (t) => {
        ...
        if (el)
            el.textContent = t; // the student may have navigated away mid-send
    };
    const refresh = (word) => {
        ...
        card.querySelector('[data-play-take]').disabled = !has;
        card.querySelector('[data-ab]').disabled = !has;
        card.querySelector('[data-rec]').textContent = has ? 're-record' : 'record';
    };
    items.forEach(refresh);
    const playTake = (word) => new Promise((resolve) => {
    ...
    onViewClick(async (e) => {
        ...
        if (rec) {
            ...
            const bar = card.querySelector('.level i');
            ...
            const ok = await recorder.start((pct) => { bar.style.width = pct + '%'; }, async (blob) => {
                takes.set(word, blob);
        ...
        const play = el.closest('[data-play-take]');
        if (play) {
            speaker.stop();
            void playTake(play.dataset.playTake);
            return;
        }
        const ab = el.closest('[data-ab]');
        if (ab) {
...
function wireSettings() {
    ...
    /* ---- VoiceStudio ---- */
    const studioVoice = $('#studio-voice');
    ...
    const commitUrl = () => {
        ...
        if (next === state.settings.studioUrl)
            return;
    ...
    studioUrl.addEventListener('keydown', (e) => { if (e.key === 'Enter') {
        e.preventDefault();
        commitUrl();
    } });
    ...
    const paint = () => {
        ...
        if (state.student)
            $('#whoami').textContent = state.student.name;
        paintOutbox();
        // Recorded clip bank
        ...
        const bankRadio = $('input[name="engine"][value="bank"]');
        ...
        if (bank.available) {
            ...
            const needed = [];
            for (const u of UNITS) {
                for (const t of u.triples)
                    needed.push(...t);
                for (const p of u.pairs)
                    needed.push(p.a, p.b);
                for (const s2 of u.sentences)
                    needed.push(plain(s2.a), plain(s2.b));
            }
        ...
        if (useS) {
            ...
            if (st.status === 'error') {
                out.hidden = false;
                out.className = 'help connout bad';
                ...
                out.hidden = false;
                out.className = 'help connout';
                out.textContent = 'Contacting VoiceStudio…';
            }
        }
        // Engine panels
    ...
    engineRadios.forEach((r) => r.addEventListener('change', () => {
        ...
        if (state.settings.engine === 'studio') {
            speaker.studio.probe(true).then(() => {
                paint();
                if (speaker.studio.ready)
                    void speaker.say('VoiceStudio is connected.');
            });
        }
    }));
    ...
    $('#conn').addEventListener('click', async () => {
        ...
        if (!reportingOn()) {
            ...
            out.textContent = 'No endpoint is set in REPORT_URL.';
            return;
        }
        btn.disabled = true;
